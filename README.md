...
## V57 decision-first operator cockpit

V57 переработал главный frontend-сценарий: оператор сначала видит короткое действие (`Не входить`, `Ждать цену`, `Пересчитать`, `Ручная проверка LONG/SHORT`), затем entry/SL/TP/RR/confidence/TTL и только после этого открывает подробности во вкладочном диалоге.

Система остается советующей: frontend не отправляет ордера автоматически и не пересчитывает рекомендацию. Источник истины — backend recommendation contract. Для проверки полноты UI-контракта добавлена миграция `20260506_v57_operator_decision_first_ui_contract.sql` и audit view `v_operator_decision_first_ui_contract_v57`, подключенная к `/api/system/warnings`. Миграция использует существующее определение active-рекомендации через `direction`, `bar_time`, `expires_at` и terminal outcomes; отдельные колонки `active`/`signal_score` не требуются.
