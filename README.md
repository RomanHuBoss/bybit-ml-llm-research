...
- Фоновые LLM-циклы защищены от перекрывающегося запуска.
...
В проверочной среде полный набор был дополнительно прогнан по файлам: `43 passed`.
...
Ожидаемый результат текущей ревизии: `43 passed`.
...
Ожидаемый результат полного прогона после обновления: `43 passed`.
...
Что остается ручным намеренно: обновление рынка, liquidity, sentiment, построение новых сигналов и ML-train. Backtest-доказательства по свежим рекомендациям поддерживаются автоматически, но ручной `POST /api/backtest/run` сохранен для точечной проверки выбранного сетапа.
