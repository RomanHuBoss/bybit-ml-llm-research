...
Система остается советующей: frontend не отправляет ордера автоматически и не пересчитывает рекомендацию. Источник истины — backend recommendation contract. Для проверки полноты UI-контракта добавлена миграция `20260506_v57_operator_decision_first_ui_contract.sql` и audit view `v_operator_decision_first_ui_contract_v57`, подключенная к `/api/system/warnings`.
