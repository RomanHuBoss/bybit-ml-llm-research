...
## Ревизия ML auto-train 2026-04-28

Проверка показала, что предыдущая версия обучала ML-модель только вручную через `/api/ml/train`. Это было неконсистентно с автоматическим контуром `universe -> market/sentiment -> signals -> backtest/LLM`: новые symbol+TF могли попадать в очередь без актуальной ML-доказательной базы.

Исправлено:

- фоновый `signal-auto-refresher` теперь после синхронизации рынка и sentiment автоматически проверяет каждую пару `category+symbol+interval+horizon`;
- если модель отсутствует, устарела по `ML_AUTO_TRAIN_TTL_HOURS` или есть запись `model_runs`, но нет локального `.joblib`-файла, выполняется переобучение;
- число ML-тренировок за цикл ограничивается `ML_AUTO_TRAIN_MAX_MODELS_PER_CYCLE`, чтобы локальный ПК/VM не перегружался;
- повторно падающие ML-тренировки притормаживаются через `ML_AUTO_TRAIN_FAILURE_COOLDOWN_HOURS`, чтобы один проблемный symbol/TF не обучался заново каждые 5 минут;
- `/api/status` и `/api/signals/background/status` возвращают параметры и итог последнего ML auto-train;
- `signals.ml_probability` теперь заполняется вероятностью именно в направлении сигнала: `P(up)` для long и `1-P(up)` для short;
- `research/rank` больше не дает вес устаревшим `model_runs`: ML-доказательство учитывается только в пределах TTL.

Ручные ML-кнопки убраны из основного интерфейса. API-endpoints `/api/ml/train` и `/api/ml/predict/latest` сохранены как аварийно-диагностический инструмент, но штатная работа больше не требует ручного обучения каждого символа.
...
7. ML-модели поддерживаются автоматически фоновым `signal-auto-refresher`; отдельный ручной шаг в UI не нужен.
...
Если при диагностическом вызове `/api/ml/train` появляется сообщение вида:
...
Что остается ручным намеренно: финальная проверка сетапа оператором и любое биржевое исполнение. Обновление рынка, sentiment, ML-доказательства и построение новых рекомендаций выполняются фоновым `signal-auto-refresher`; ML endpoints сохранены для API-диагностики, но не вынесены в основной UI.
