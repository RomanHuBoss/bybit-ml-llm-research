...
В проверочной среде полный набор был дополнительно прогнан по файлам: `47 passed`.
...
4. `Build signals` — построить rule-based сигналы вручную, если нужно немедленно.
5. Авто-контур `signal-auto-refresher` делает шаги 1–4 периодически сам, если включен `SIGNAL_AUTO_REFRESH_ENABLED=true`.
...
```text
liquidity universe -> market sync -> sentiment sync -> build/persist signals -> wake backtest + LLM
```
...
- свечи/funding/OI догружаются коротким окном `SIGNAL_AUTO_SYNC_DAYS`, чтобы цикл был эксплуатационно ограниченным;
...
SIGNAL_AUTO_MAX_SYMBOLS=12
SIGNAL_AUTO_SYNC_DAYS=7
