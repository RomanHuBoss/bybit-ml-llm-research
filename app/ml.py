...
def model_path(category: str, symbol: str, interval: str, horizon_bars: int) -> str:
    safe = f"{category}_{symbol.upper()}_{interval}_{horizon_bars}.joblib".replace("/", "_")
    return str(MODELS_DIR / safe)


def train_model(category: str, symbol: str, interval: str, horizon_bars: int = 12) -> dict[str, Any]:
    X, y, df = build_ml_dataset(category, symbol, interval, horizon_bars)
    if len(X) < 500:
        raise ValueError("Need at least 500 feature rows. Sync more history first.")
    ...
    try:
        perm = permutation_importance(pipe, X_test, y_test, n_repeats=8, random_state=42, scoring="roc_auc")
    ...
    execute_many_values(
        ...
        [
            (
                ...
                metrics["roc_auc"],
                importance,
                {"threshold": 0.55, "model_path": path},
            )
        ],
    )
