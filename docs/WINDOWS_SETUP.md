...
Важно: интерфейс не отправляет ордера и не отправляет ордера автоматически. Статус `ПРОВЕРИТЬ` означает только, что кандидата можно передать оператору на ручную проверку стакана, новостей, риска портфеля и актуальности цены.
...
2. Сверху в центре показан итоговый статус выбранного сетапа: `НЕТ ВХОДА`, `НАБЛЮДАТЬ`, `РУЧНАЯ ПРОВЕРКА ВХОДА`.
...
Система по-прежнему не отправляет ордера и не отправляет ордера автоматически.
...
Если Ollama не запущен, приложение продолжает работать, а LLM-статус будет показывать ошибку фонового анализа. Это не включает live-trading и не отправляет ордера автоматически.
