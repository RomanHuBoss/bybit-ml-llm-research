...
- любые красные пункты чек‑листа явно запрещают передачу сетапа на ручную проверку входа;
...
Безопасное допущение: интерфейс намеренно не пытается сделать кнопку «войти». Даже статус `РУЧНАЯ ПРОВЕРКА ВХОДА` означает только допуск к ручной проверке оператором.
